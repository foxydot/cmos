...
add_image_size( 'featured-widget', 60, 60, TRUE );
add_image_size( 'featured', 550, 300, TRUE );
...
function tapestry_remove_elements() {
	...
	if ( get_post_format() ) {
		...
		remove_action( 'genesis_before_post_content','tapestry_do_post_image');
		if(!get_post_format()=="image"){
			add_action( 'genesis_before_post_content','tapestry_do_post_image');
		} 
...
function tapestry_do_post_image() {
		...
		printf( '<a href="%s" title="%s">%s</a><div class="clear"></div>', get_permalink(), the_title_attribute('echo=0'), $img );
}
